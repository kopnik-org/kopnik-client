...
export default {
  ...
  methods: {
    ...
    lmap_zoomstart(event) {
      // Управзднить в тестах это событие срабатывает, а вот последующее updateBounds нет.
      // Это приводит к тому отменяет загрузку маркеров на карте, и потом она не начинается никогда
      if (event.target._size.x===0 && event.target._size.y===0){
        return
      }
    ...
    lmap_updateBounds(event) {
      // это в тестах так заряжается
      if (event._northEast.equals(event._southWest)) {
        event._northEast= new LatLng(+40, +180)
        event._southWest= new LatLng(-40, -180)
      }
      // фиксим ошибку vue-leaflet, который выбрасывает два события подряд
